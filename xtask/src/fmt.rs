...
use super::{FmtCli as Cli, FmtFileType};
...
impl Cli {
    fn creat_cmd_contexts(&self) -> Result<Vec<Box<dyn CommandContext>>> {
        ...
        let command_context = file_types
            .iter()
            .map(
                // TODO: remove the extra "+ 'static", once
                // https://github.com/rust-lang/rust-clippy/issues/11417 is fixed.
                |file_type| -> Result<Vec<Box<dyn CommandContext + 'static>>> {
                    match file_type {
...
pub(crate) fn main(mut cli: Cli) -> Result<()> {
    let cli = {
